...
- **Breaking (`estimate_betas`), aligned with r-tidyfinance:** coefficient columns are now named `intercept` and `beta_<variable>` (was `Intercept` and the bare variable name), and the identifier and date columns come first, so the output is `<id>, date, intercept, beta_<variable>, ...`. The `lookback` argument is unchanged (an integer count of consecutive observations).
