...
def test_first_breakpoint_equals_min_last_approximately_equals_max():
    ...
    bp = compute_breakpoints(data, "value", breakpoint_options(n_portfolios=5))
    assert bp[0] == data.get_column("value").min()
    # Last breakpoint = max + tiny epsilon (1e-20)
    assert abs(bp[5] - (data.get_column("value").max() + 1e-20)) < 1e-15
...
def test_percentile_breakpoints_are_ascending():
    ...
    data = pl.DataFrame({"id": range(500), "value": rng.standard_normal(500)})
    bp = compute_breakpoints(
        data,
        "value",
...
def test_interior_breakpoints_are_slightly_larger_than_raw_quantiles():
    ...
    bp = compute_breakpoints(data, "value", breakpoint_options(n_portfolios=5))
    raw_q = np.quantile(
        data.get_column("value").to_numpy(), np.linspace(0, 1, 6)
    )
    assert bp[0] == raw_q[0]
    for i in range(1, 6):
...
def test_na_values_in_sorting_variable_are_ignored():
    """Test missing values in sorting variable are ignored."""
    data_clean = pl.DataFrame(
        {"id": range(100), "value": np.arange(1, 101).astype(float)}
    )
    data_na = pl.DataFrame(
        {
            "id": range(120),
            "value": list(np.arange(1, 101).astype(float)) + [None] * 20,
        }
    )
...
def test_works_with_very_small_data():
    ...
    data = pl.DataFrame({"id": [1, 2], "value": [1.0, 10.0]})
    bp = compute_breakpoints(data, "value", breakpoint_options(n_portfolios=2))
    assert len(bp) == 3
    assert bp[0] == 1
...
def test_works_with_n_3_and_3_portfolios():
    ...
    data = pl.DataFrame({"id": [1, 2, 3], "value": [1.0, 5.0, 10.0]})
...
def test_breakpoints_produce_valid_portfolio_assignments():
    ...
    valid = portfolios.drop_nulls()
    assert valid.is_between(1, 5).all()
    assert sorted(valid.unique().to_list()) == [1, 2, 3, 4, 5]
...
def test_min_size_threshold_produces_correct_breakpoints():
    ...
    data = pl.DataFrame({"mktcap_lag": mktcap, "sorting_var": sorting_var})
...
def test_min_size_threshold_with_na_mktcap_excludes_na_rows():
    """Test breakpoints_min_size_threshold with null mktcap excludes null rows."""
    data = pl.DataFrame(
        {
            "mktcap_lag": [None, 10, 20, 30, 40, 50, 60, 70, 80, 90],
    ...
    mktcap_all = data.get_column("mktcap_lag").cast(pl.Float64).to_numpy()
    above = ~np.isnan(mktcap_all) & (mktcap_all > size_cutoff)
    expected = np.quantile(
        data.get_column("sorting_var").to_numpy()[above],
        np.linspace(0, 1, 4),
    )
    expected[1:] += 1e-20
    np.testing.assert_allclose(bp, expected)
